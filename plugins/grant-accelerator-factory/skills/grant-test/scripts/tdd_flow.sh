#!/usr/bin/env bash
# tdd_flow.sh — guided test-first git flow for Grant Accelerator engagements.
#
...
#
# Branch convention:  feature/<client>-<slug>   (e.g. feature/acme-add-report)
...
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"        # repo root (skill is .claude/skills/grant-test)
[ -d "$ROOT/product/pipeline/testkit" ] || ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
TESTKIT="$ROOT/product/pipeline/testkit"
RUN="python3 $TESTKIT/grant_test.py"
...
compute_tier() {  # prints "TIER=n" then one reason per line; returns 0
  ...
  if env | grep -qE '^(GRANT|PSS|LPI|FI)_SKIP_[A-Z_]+='; then
    tier=3; reasons+=("skip variable set: $(env | grep -oE '^(GRANT|PSS|LPI|FI)_SKIP_[A-Z_]+' | tr '\n' ' ')")
  fi
...
case "$cmd" in
  ...
  commit)
    ...
    git -C "$ROOT" commit -m "[$client] $msg" \
      -m "Gate: PASS (grant-test $( [ -n "$org" ] && echo "full/$org" || echo static )) | Tier: $t" \
      -m "$(cat "$mrp")"
