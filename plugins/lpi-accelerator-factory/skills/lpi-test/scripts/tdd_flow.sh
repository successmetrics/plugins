#!/usr/bin/env bash
# tdd_flow.sh — guided test-first git flow for LPI engagements.
#
...
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"        # repo root (skill is .claude/skills/lpi-test)
[ -d "$ROOT/product/pipeline/testkit" ] || ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
TESTKIT="$ROOT/product/pipeline/testkit"
RUN="python3 $TESTKIT/lpi_test.py"
...
compute_tier() {  # prints "TIER=n" then one reason per line; returns 0
  ...
  if env | grep -qE '^(LPI|PSS|GRANT|FI)_SKIP_[A-Z_]+='; then
    tier=3; reasons+=("skip variable set: $(env | grep -oE '^(LPI|PSS|GRANT|FI)_SKIP_[A-Z_]+' | tr '\n' ' ')")
  fi
...
run_gate() {  # args: [client] [org]
  ...
  # API-surface gate first (2026-08-23): the base's global Apex / CMDT / hook surface
  # must match the committed snapshot BEFORE anything commits. Breaking drift = MAJOR-bump
  # event; additive drift = regenerate+commit the snapshot with the change. Source-only,
  # cheap, so it runs on every gate invocation.
  echo "▶ gate: api-surface --check"
  python3 "$ROOT/product/pipeline/check_api_surface.py" --check || return 1
  # Minimum-org-profile gate (2026-09-01): no packaged reference outside the base-only PSS
  # floor (product/pipeline/min_org_profile.yaml, measured from the reference base-only org). Source-only, cheap.
  # Four customer-install failures in a row were each a reference our richer build org masked.
  echo "▶ gate: min-org-profile --static"
  python3 "$ROOT/product/pipeline/check_build_baseline.py" --static || return 1
  local args=(--quiet)
...
case "$cmd" in
  ...
  commit)
    ...
    git -C "$ROOT" commit -m "[$client] $msg" \
      -m "Gate: PASS (lpi-test $( [ -n "$org" ] && echo "full/$org" || echo static )) | Tier: $t" \
      -m "$(cat "$mrp")"
